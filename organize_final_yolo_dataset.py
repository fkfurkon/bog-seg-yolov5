...
def copy_files_with_labels(file_list, source_image_dir, source_label_dir, dest_image_dir, dest_label_dir, split_name):
...
def organize_yolo_dataset(merged_dataset_path, image_dir, output_root, val_ratio=0.2):
    ...
    train_count, train_missing = copy_files_with_labels(
        train_files, image_dir, source_label_dir, output_path / "images/train", output_path / "labels/train", "Train"
    )

    # Copy val files
    val_count, val_missing = copy_files_with_labels(
        val_files, image_dir, source_label_dir, output_path / "images/val", output_path / "labels/val", "Val"
    )
...
def main():
    ...
    parser.add_argument("--merged-dataset", default="merged_dataset_final", help="Path to merged dataset directory")
    parser.add_argument("--image-dir", default="dataset/image", help="Path to images directory")
    parser.add_argument("--output", default="yolo_dataset_final", help="Output directory for organized YOLO dataset")
    ...
    organize_yolo_dataset(args.merged_dataset, args.image_dir, args.output, args.val_ratio)
