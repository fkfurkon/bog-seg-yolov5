...
class MergedDatasetInference:
    ...
    def visualize_comparison(self, image, predictions, ground_truth, save_path=None):
        ...
        for gt in ground_truth:
            ...
            if len(coords) >= 6:  # At least 3 points
                ...
                if len(points) > 0:
                    ...
                    cv2.putText(gt_image, label, tuple(points[0]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        ...
        for pred in predictions:
            ...
            cv2.putText(pred_image, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        ...
        # Add titles
        cv2.putText(
            comparison, "Ground Truth (Merged Annotations)", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2
        )
        cv2.putText(comparison, "Model Predictions", (width + 10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
def run_merged_inference():
    ...
    for idx, img_path in enumerate(sample_images[:6]):
        ...
        try:
            ...
            inference.visualize_comparison(image, predictions, ground_truth, output_path)
    ...
    print("   ✓ Merged annotations (bbox + segmentation)")
    print("   ✓ Multi-class food recognition")
    print("   ✓ Thai food categories with distribution:")
    print("     • protein (41.2%)")
    print("     • vegetable (39.0%)")
    print("     • carbohydrate (9.5%)")
    print("     • flatware (4.2%)")
    print("     • Others (6.1%)")
...
def create_merged_summary(results, output_dir):
    ...
    for i, img in enumerate(images):
        if i < len(axes):
            ...
            axes[i].set_title(f"Example {i + 1}: GT={gt_count} | Pred={pred_count}", fontsize=12, fontweight="bold")
            axes[i].axis("off")
