...
class YOLOSegmentationInference:
    def __init__(self, weights_path, data_config):
        ...
        try:
            ...
            print("⚠️  Using demo weights (trained model not found)")
...
def run_inference_examples():
    ...
    for idx, img_path in enumerate(sample_images[:6]):
        ...
        try:
            ...
            results.append({"image": str(img_path), "output": str(output_path), "detections": len(predictions)})
...
def create_inference_summary(results, output_dir):
    ...
    for i, img in enumerate(images):
        if i < len(axes):
            ...
            axes[i].set_title(f"Example {i + 1}: {detections} food items detected", fontsize=12, fontweight="bold")
            axes[i].axis("off")
