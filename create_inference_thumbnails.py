...
def create_food_inference_examples(dataset_dir='dataset/image', output_dir='runs/inference_examples', num_examples=6):
    ...
    print(f"📊 Each example shows:")
    print(f"   • Detected bounding boxes with class labels")
    print(f"   • Instance segmentation masks (semi-transparent overlay)")
    print(f"   • Confidence scores for each detection")
    print(f"   • Thai and English class names")
...
def create_inference_summary(results, output_dir):
    ...
    fig.suptitle('YOLOv5 Instance Segmentation - Food Dataset Inference Examples\n'
                 'Bounding Boxes + Segmentation Masks Overlay', fontsize=16, fontweight='bold')
    ...
    for i, img in enumerate(images):
        if i < len(axes):
            axes[i].imshow(img)
            axes[i].set_title(f'Example {i+1}: {results[i]["predictions"]} objects detected',
...
if __name__ == '__main__':
    ...
    print(f"✓ Instance segmentation masks")
    print(f"✓ Class labels in Thai and English")
    print(f"✓ Confidence scores")
    print(f"✓ 9 food categories: protein, carbohydrate, fruit, dessert, flatware, vegetable, sauce, soup, snack")
