        ...
        try:
            if Path(weights_path).exists():
                ...
                self.model = torch.hub.load('ultralytics/yolov5', 'yolov5s-seg', 
                                          device=self.device)
                print(f"⚠️  Using pretrained model (trained model not found)")
...
def run_merged_inference_examples():
    ...
    # Get sample images
    val_images = list(Path("merged_dataset/images/val").glob("*.jpg")) + \
                 list(Path("merged_dataset/images/val").glob("*.png"))
    
    if not val_images:
        # Fallback to train images
        val_images = list(Path("merged_dataset/images/train").glob("*.jpg")) + \
    ...
    for idx, img_path in enumerate(sample_images):
        ...
        try:
            ...
            result_image = inference.visualize_results(image, detections, output_path)
    ...
    print(f"   ✓ Combined bounding box + segmentation training")
    print(f"   ✓ Multi-class Thai food detection")
    print(f"   ✓ Thai and English labels")
    print(f"   ✓ High-quality merged annotations")
...
def create_merged_summary(results, output_dir, inference):
    ...
    fig.suptitle('YOLOv5 Segmentation - Merged Dataset Results\n'
