...
def create_merged_demo_visualization():
    ...
    # Get sample images
    train_images = list(Path("merged_dataset/images/train").glob("*.jpg")) + list(
    ...
    for idx, img_path in enumerate(sample_images):
                ...
                for line in f:
                    ...
                    if len(parts) >= 7:  # class_id + at least 6 coordinates
                        ...
                        if len(pixel_coords) >= 6:
                            detections.append({"class_id": class_id, "coords": pixel_coords})
        ...
        for detection in detections:
            ...
            if len(coords) >= 6:
                ...
                # Background for text
                cv2.rectangle(
                    result_image, (x_min, y_min - text_height - baseline - 5), (x_min + text_width, y_min), color, -1
                )
        ...
        results.append({"image": str(img_path), "output": str(output_path), "detections": len(detections)})
    ...
    print("   ✓ Merged bounding box + segmentation annotations")
    print("   ✓ Polygon segmentation masks")
    print("   ✓ Bounding box overlays")
    print("   ✓ Thai and English labels")
    print("   ✓ 9 food classes")
...
def create_demo_summary_grid(results, output_dir):
    ...
    fig.suptitle(
        "Merged Dataset Visualization - Bounding Box + Segmentation\n"
    ...
    for i, img in enumerate(images):
        if i < len(axes):
            ...
            axes[i].set_title(
                f"Example {i + 1}: {detections} food items\nBbox + Polygon Annotations", fontsize=11, fontweight="bold"
            )
            axes[i].axis("off")
...
def show_annotation_stats():
    ...
    thai_names = ["โปรตีน", "คาร์โบไฮเดรต", "ผลไม้", "ของหวาน", "ช้อนส้อม", "ผัก", "ซอส", "ซุป", "ขนม"]
...
if __name__ == "__main__":
    ...
    print("📈 Monitor training: tensorboard --logdir runs/train-seg")
