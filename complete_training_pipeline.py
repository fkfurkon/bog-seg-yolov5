...
def create_sample_inference(weights, source_dir, output_dir, device=''):
    ...
    cmd = [
        sys.executable, 'instance_segmentation_inference.py',
        '--weights', str(weights),
...
def create_results_summary(model_dir, val_output, inference_dir):
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"- Model: YOLOv5s-seg\n")
        f.write(f"- Dataset: Food segmentation dataset\n")
        f.write(f"- Classes: 9 (protein, carbohydrate, fruit, dessert, flatware, vegetable, sauce, soup, snack)\n\n")
        ...
        f.write("2. Run inference: `python instance_segmentation_inference.py --weights runs/train-seg/food_segmentation/weights/best.pt --source path/to/images`\n")
        f.write("3. Validate model: `python segment/val.py --weights runs/train-seg/food_segmentation/weights/best.pt --data food_dataset.yaml`\n\n")
