    ...
    def predict_and_visualize(self, img_path, save_path=None):
        ...
        if len(results[0]['boxes']) > 0:
            ...
            for i, (box, score, class_id) in enumerate(zip(
                results[0]['boxes'], results[0]['scores'], results[0]['classes']
            )):
                class_name = self.names[class_id]
...
def create_comprehensive_thumbnail_grid(segmentation_engine, image_paths, output_path):
    ...
    for i, img_path in enumerate(image_paths[:4]):
        ...
        axes[row, col].set_title(
            ...
            f"จำนวนวัตถุที่ตรวจพบ: {len(results[0]['boxes'])} objects",
            fontsize=14,
            fontweight='bold'
        )
    ...
    for class_id, class_name in segmentation_engine.names.items():
        color = np.array(segmentation_engine.class_colors.get(class_id, (128, 128, 128))) / 255
...
def main():
    ...
    parser.add_argument('--weights', 
                       default='runs/train/food_instance_segmentation/weights/best.pt',
                       help='Path to trained weights')
    parser.add_argument('--source', 
                       default='yolo_dataset_final/images/val',
                       help='Source directory or image file')
    parser.add_argument('--output', 
                       default='runs/inference/enhanced_instance_segmentation',
